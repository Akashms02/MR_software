...
export const CompanyTerminationLetter = (employeeId, payload) => async (dispatch) => {
  dispatch({ type: LOADING_START });
  ...
  try {
    ...
    if (isSuccess(status) || response.status === 200) {
      dispatch({
        type: COMPANY_TERMINATION_LETTER_GENERATE_SUCCESS,
        payload: data || response.data.data || [],
      });
      return response.data;
    }
    ...
    dispatch({
      type: COMPANY_TERMINATION_LETTER_GENERATE_FAILURE,
      payload: message || commonError,
    });
  } catch (error) {
    dispatch({
