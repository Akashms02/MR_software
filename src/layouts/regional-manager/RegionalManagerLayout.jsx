...
export default function RegionalManagerLayout({ children }) {
  const navigate = useNavigate()
  const location = useLocation()
  const dispatch = useDispatch()
  const { user } = useSelector(state => state.auth)
  const displayName = user?.fullName || user?.name || user?.email?.split('@')[0] || 'Regional Manager'
  const displayRole = 'Regional Manager'
  const pathParts = location.pathname.split('/')
  const activePage = pathParts[pathParts.length - 1] || 'dashboard'
  const navItems = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'Dashboard', path: '/regional-manager/dashboard' },
    { id: 'finance', icon: FileText, label: 'My Payslips', path: '/regional-manager/finance' },
    { id: 'me', icon: User, label: 'Me', path: '/regional-manager/me' },
