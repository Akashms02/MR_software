...
export default function DistributorLayout({ children }) {
  ...
  const displayName = user?.fullName || user?.name || user?.email?.split('@')[0] || 'Distributor'
  const displayRole = 'Distributor'

  const pathParts = location.pathname.split('/')
  const activePage = pathParts[pathParts.length - 1] || 'dashboard'

  const navItems = [
    { id: 'dashboard',    icon: LayoutDashboard, label: 'Dashboard',     path: '/distributor/dashboard' },
    { id: 'finance',      icon: FileText,        label: 'My Payslips',   path: '/distributor/finance' },
    { id: 'me',           icon: User,            label: 'Me',            path: '/distributor/me' },
    { id: 'watercooler',  icon: Coffee,          label: 'Water Cooler',  path: '/distributor/watercooler' },
    { id: 'settings',     icon: Settings,        label: 'Settings',      path: '/distributor/settings' },
  ]
