...
export default function AreaManagerLayout({ children }) {
  const navigate = useNavigate()
  const location = useLocation()
  const dispatch = useDispatch()
  const { user } = useSelector(state => state.auth)

  const displayName = user?.fullName || user?.name || user?.email?.split('@')[0] || 'Area Manager'
  const displayRole = 'Area Manager'

  const pathParts = location.pathname.split('/')
  const activePage = pathParts[pathParts.length - 1] || 'dashboard'

  const navItems = [
    { id: 'dashboard',    icon: LayoutDashboard, label: 'Dashboard',     path: '/area-manager/dashboard' },
    { id: 'finance',      icon: FileText,        label: 'My Payslips',   path: '/area-manager/finance' },
    { id: 'me',           icon: User,            label: 'Me',            path: '/area-manager/me' },
    { id: 'watercooler',  icon: Coffee,          label: 'Water Cooler',  path: '/area-manager/watercooler' },
    { id: 'settings',     icon: Settings,        label: 'Settings',      path: '/area-manager/settings' },
  ]
