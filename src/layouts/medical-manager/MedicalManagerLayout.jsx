...
function SidebarAvatar({ name, size = 38 }) {
  ...
  return (
    <div 
      ...
      className="rounded-full bg-gradient-to-br from-[#A78BFA] 0% to-[#7C3AED] 100% flex items-center justify-center text-white font-bold shrink-0 overflow-hidden"
    >
      {initials}
    </div>
  )
...
export default function MedicalManagerLayout({ children }) {
  ...
  const filteredNavItems = navItems.filter(item => {
    ...
    if (targetId === 'onboard-doctor') targetId = 'requests';
    ...
    <div className="flex min-h-screen bg-[#F0F2F5] font-sans">
      <aside className="w-[220px] shrink-0 bg-white flex flex-col fixed top-0 left-0 bottom-0 z-[100] shadow-[2px_0_16px_rgba(0,0,0,0.04)]">
        ...
        <div className="h-[72px] flex items-center px-5 gap-2.5 shrink-0 border-b border-[#F3F4F6]">
          <div className="w-9 h-9 rounded-xl bg-gray-905 bg-gray-900 flex items-center justify-center shrink-0">
            <SnowflakeLogo />
          </div>
