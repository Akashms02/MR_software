...
export default function HRLayout({ children }) {
  ...
  const displayName = user?.fullName || user?.name || user?.email?.split('@')[0] || 'HR Employee'
  const displayRole = 'HR Manager (HR)'
  ...
  const navItems = [
    ...
    { id: 'finance', icon: FileText, label: 'My Payslips', path: '/hr/finance' },
    { id: 'me', icon: User, label: 'Me', path: '/hr/me' },
    { id: 'watercooler', icon: Coffee, label: 'Water Cooler', path: '/hr/watercooler' },
    { id: 'settings', icon: Settings, label: 'Settings', path: '/hr/settings' },
  ]
  return (
