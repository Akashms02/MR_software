...
export default function ManagerLayout({ children }) {
  ...
  const displayName = user?.fullName || user?.name || user?.email?.split('@')[0] || 'Supervisor'
  ...
  const handleNotificationClick = (notification) => {
    ...
    if (typeLower.includes('leave') || titleLower.includes('leave') || descLower.includes('leave')) {
      ...
      navigate('/manager/tourplans');
    } else if (typeLower.includes('dcr') || titleLower.includes('dcr') || descLower.includes('dcr')) {
      navigate('/manager/dcr-approvals');
      ...
      <aside className="w-[220px] shrink-0 bg-white flex flex-col fixed top-0 left-0 bottom-0 z-[100] shadow-[2px_0_16px_rgba(0,0,0,0.04)]">
        ...
        <nav className="flex-1 overflow-y-auto p-3">
          {filteredNavItems.map(item => {
            // Match active state (handling subroutes under myteam)
            const isActive = activePage === item.id || (item.id === 'myteam' && location.pathname.includes('/myteam'))
        ...
        <div className="p-3 border-t border-[#F3F4F6] shrink-0">
          <div className="flex items-center gap-2.5 p-2.5 rounded-xl mb-1 bg-gray-50">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-gray-400 truncate">{displayRole}</div>
            </div>
          </div>
          <button
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {notifications.length === 0 ? (
                  ...
                  notifications.map(notification => {
                    ...
                    if (typeLower.includes('leave') || titleLower.includes('leave')) {
                    ...
                    } else if (typeLower.includes('dcr') || titleLower.includes('dcr')) {
