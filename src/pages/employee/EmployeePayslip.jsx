...
export default function EmployeePayslip() {
  ...
  const [previewUrl, setPreviewUrl] = useState(null); // Set API response PDF/image URL here later
  const [downloading, setDownloading] = useState(false);
  const printableRef = useRef(null);

  const empName = user?.fullName || user?.name || 'Akash Kumar';
  const empId = user?.employeeId || user?.id || 'GH002';
  const designation = user?.designation || 'Medical Representative';
  const department = user?.department || 'Sales & Marketing';
  const salary = user?.salary || user?.salaryAmount || 35000;
  ...
  const handleDownload = async () => {
    ...
    try {
      // Override computed styles dynamically to resolve OKLCH/OKLAB colors for html2canvas
      ...
      const opt = {
        margin: [0, 0, 0, 0],
        filename: `payslip_${empName.replace(/\s+/g, '_').toLowerCase()}_${formattedMonth}.pdf`,
        ...
        html2canvas: { 
          ...
          onclone: (clonedDoc) => {
            ...
            // Override getComputedStyle inside the cloned iframe
            ...
            // Replace all oklch references in iframe styles
    ...
    <div className="animate-[fadeSlideIn_0.35s_ease-out] flex flex-col h-[calc(100vh-104px)] min-h-0 overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-4 shrink-0 flex-wrap gap-3">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-[18px] py-2 rounded-xl border-none bg-[#C8F04A] text-[#111827] font-extrabold text-[12.5px] cursor-pointer shadow-[0_4px_12px_rgba(200,240,74,0.2)] hover:opacity-90 transition-opacity duration-150 outline-none disabled:opacity-60 disabled:cursor-not-allowed"
        >
