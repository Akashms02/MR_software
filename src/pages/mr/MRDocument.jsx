...
export default function MRDocument() {
  ...
  const empName = user?.fullName || user?.name || 'Medical Representative';
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      const filename = selectedDocType === 'payslip'
        ? `payslip_mr_${empName.replace(/\s+/g, '_').toLowerCase()}_${formattedMonth}.pdf`
        : `${selectedDocType}_letter_mr_${empName.replace(/\s+/g, '_').toLowerCase()}.pdf`;
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="px-3.5 py-2 rounded-xl border border-gray-255 text-[13px] bg-white outline-none font-bold text-[#111827] focus:border-[#C8F04A] transition-colors duration-150 cursor-pointer"
            >
