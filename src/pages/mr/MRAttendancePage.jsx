...
import { Clock, Square, MapPin, Calendar, X, Loader2 } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1500] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl w-full max-w-xl max-h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in scale-in duration-200">
        ...
        <div className="px-6 py-5 flex items-start justify-between border-b border-slate-100 bg-slate-50/50">
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-655 transition-colors border-none bg-transparent cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1 min-h-0 space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12 text-slate-400">
              ...
              <p className="m-0 text-xs mt-1">There are no other completed visits recorded for this location.</p>
            </div>
          ) : (
      ...
      <div className="flex justify-end items-center gap-3 mb-4 shrink-0">
        ...
        <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border border-gray-200 shadow-[0_2px_6px_rgba(0,0,0,0.02)]">
          ...
          <select
            ...
            className="px-2.5 py-1.5 rounded-lg border border-gray-300 text-[13px] bg-gray-50 font-bold text-gray-850 cursor-pointer outline-none font-sans disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <option value="">
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start border-b border-gray-100 pb-2 mb-2.5">
                            <div>
                              ...
                              <button
                                onClick={() => setHistoryTarget({ id: v.targetId || v.id, name: v.name, type: v.type })}
                                className="mt-1 px-2.5 py-1 rounded bg-slate-100 hover:bg-slate-200 text-slate-655 hover:text-slate-800 text-[10.5px] font-bold inline-flex items-center gap-1 border-none cursor-pointer transition-all active:scale-95"
                              >
                                🔍 View History
                              </button>
                            </div>
