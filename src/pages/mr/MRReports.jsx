...
export default function MRReports() {
  ...
  // Re-fetch report when category, or date filters change (reset page to 1)
  useEffect(() => {
    setPage(1);
    handleFetchReport(1);
    ...
    return (
      <div className="flex items-center justify-between px-5 py-3.5 bg-white border-t border-gray-150 rounded-b-2xl">
        <div className="text-[12px] text-gray-500 font-semibold">
          Showing Page <span className="font-bold text-gray-800">{paginator.currentPage}</span> of <span className="font-bold text-gray-800">{paginator.pageCount}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => setPage(p => Math.max(1, p - 1))}
            disabled={paginator.currentPage <= 1}
            className="px-3 py-1.5 rounded-lg text-[11px] font-bold border border-gray-200 bg-white text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-all duration-150"
          >
            Previous
          </button>
          <button
  ...
  return (
    <div className="animate-[fadeIn_0.35s_ease-out] font-sans flex flex-col gap-6">
      ...
      <Card className="p-5 flex flex-col gap-5">
        ...
        <div>
          <h3 className="text-[11px] font-extrabold text-[#9CA3AF] uppercase tracking-wider mt-0 mb-3 ml-1">
            Report Category
          </h3>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border border-transparent cursor-pointer font-sans transition-all duration-150 text-[13px] ${
                    isActive 
                      ? 'bg-[#C8F04A] text-[#1A1A1A] font-bold shadow-[0_4px_12px_rgba(200,240,74,0.2)]' 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 items-end border-t border-gray-100 pt-4.5">

          {/* Date range pickers */}
          {(activeReport === 'visit-summary' || activeReport === 'datewise-daily' || activeReport === 'call-visit') && (
            <>
              <div className="flex flex-col gap-1.5">
                <label className="block text-[11px] font-bold text-[#4B5563] uppercase tracking-wider">Start Date</label>
                <input 
                  type="date" 
                  value={startDate}
              ...
              <div className="flex flex-col gap-1.5">
                <label className="block text-[11px] font-bold text-[#4B5563] uppercase tracking-wider">End Date</label>
                <input 
                  type="date" 
                  value={endDate}
          ...
          {(activeReport === 'dcr-day' || activeReport === 'daily-activity') && (
            <div className="flex flex-col gap-1.5">
              <label className="block text-[11px] font-bold text-[#4B5563] uppercase tracking-wider">Select Date</label>
              <input 
                type="date" 
                value={date}
          ...
          {activeReport === 'weekly-cross' && (
            <div className="flex flex-col gap-1.5">
              <label className="block text-[11px] font-bold text-[#4B5563] uppercase tracking-wider">Date In Week</label>
              <input 
                type="date" 
                value={dateInWeek}
      ...
      <div className="flex flex-col gap-6 relative min-h-[300px]">
        ...
        {activeReport === 'visit-summary' && hasData() && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <Card className="p-5 border-l-[5px] border-l-[#3B82F6] flex flex-col justify-between">
              ...
              <Card className="p-5 border-l-[5px] border-l-[#10B981] flex flex-col justify-between">
              ...
              <Card className="p-5 border-l-[5px] border-l-[#8B5CF6] flex flex-col justify-between">
              ...
              <Card className="p-5 border-l-[5px] border-l-[#C8F04A] flex flex-col justify-between">
        ...
        {activeReport === 'dcr-day' && hasData() && (
          <>
            ...
            <Card className="p-6">
              <div className="flex justify-between items-start border-b border-gray-100 pb-[18px] mb-[18px]">
                <div>
                  ...
                  <h3 className="text-lg font-extrabold text-gray-900 mt-2 mb-0.5">Daily Call Report Sheet</h3>
                  <p className="text-xs text-gray-500 m-0">Date: <strong>{currentData.date}</strong></p>
                </div>
                <div className="text-right">
                  <div className="text-xs text-gray-500">Verified Status</div>
                  <div className="text-[13px] font-bold text-gray-900 mt-0.5">{currentData.status || 'Pending'}</div>
                </div>
              </div>

              {/* Manager Comments */}
              {currentData.managerRemarks && (
                <div className="bg-[#F9FAFB] p-4 rounded-xl border-l-4 border-l-[#10B981] text-[13px] text-gray-600 mb-5 italic">
                  "{currentData.managerRemarks}"
                </div>
              )}
            ...
            {currentData.doctorsMet && currentData.doctorsMet.length > 0 && (
              <Card className="p-6">
                <h3 className="m-0 mb-4 text-[14.5px] font-extrabold text-gray-800">Visited Doctor Records</h3>
        ...
        {activeReport === 'daily-activity' && hasData() && (
          <>
            ...
            <Card className="p-6">
              <h3 className="text-[16px] font-extrabold text-gray-900 m-0 mb-4">Daily Activity Verification Checklist</h3>
